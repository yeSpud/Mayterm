 ...
 * @author Joren Six
 */
public final class CustomBlockingAudioPlayer implements CustomAudioProcessor {
	...
	/**
	 * The overlap and step size defined not in samples but in bytes. So it depends
	 * on the bit depth. Since the integer data type is used only 8,16,24,... bits
	 * or 1,2,3,... bytes are supported.
	 */
	 ...
	 * @param bufferSize The size of each buffer in samples (not in bytes).
	 * @param overlap    Defines how much consecutive buffers overlap in samples
	 *                   (not in bytes).
	 * @throws LineUnavailableException If no output line is available.
	 */
	public CustomBlockingAudioPlayer(final AudioFormat format, final int bufferSize, final int overlap)
		...
		// I dont actually want it to play anything, so I commented out the start :P
		// line.start();
	 ...
	 * (non-Javadoc)
	 * 
	 * @see be.hogent.tarsos.util.RealTimeAudioProcessor.AudioProcessor#processFull
	 * (float[], byte[])
	 */
	 ...
	 * (non-Javadoc)
	 * 
	 * @see be.hogent.tarsos.util.RealTimeAudioProcessor.AudioProcessor#proccess(
	 * float[], byte[])
	 */

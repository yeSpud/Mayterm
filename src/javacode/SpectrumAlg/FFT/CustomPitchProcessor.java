 ...
 * @author Joren Six
 */
public class CustomPitchProcessor implements CustomAudioProcessor {
	...
	public interface DetectedPitchHandler {
		 ...
		 * 
		 * @param pitch       The pitch in Hz. -1 is returned if no pitch is detected.
		 * @param probability The probability (a value between 0 and 1) represents how
		 *                    periodic the signal is.
		 * @param timeStamp   A time stamp associated with the detection.
		 * @param progress    If the length of the stream is known beforehand (a file) a
	 ...
	 * @param bufferOverlap        The size of the overlap between two consecutive
	 *                             buffers (in samples).
	 * @param totalLengthInSamples The total length of the stream (in samples).
	 * @param handler              The handler handles detected pitch.
	 */
	public CustomPitchProcessor(PitchEstimationAlgorithm algorithm, float sampleRate, int bufferSize, int bufferOverlap,
			long totalLengthInSamples, DetectedPitchHandler handler) {
	...
	public boolean processFull(float[] audioFloatBuffer, byte[] audioByteBuffer) {
		...
		float pitch = detector.getPitch(audioFloatBuffer).getPitch();
		float probability = detector.getPitch(audioFloatBuffer).getProbability();
